...
export default function DeleteMealPlanModal({
  mealPlan,
  onConfirm,
  onClose,
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      setErrorMessage(getErrorMessage(err, 'Failed to delete meal plan.'));
    } finally {
      ...
      <div className="modal-box w-full sm:max-w-md flex flex-col max-h-[85dvh] p-0 gap-0 overflow-hidden">
        <div className="shrink-0 border-b border-base-200 px-4 pt-4 pb-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-bold text-base text-base-content">Delete Meal Plan</h3>
              <p className="text-sm text-base-content/60">
                Remove this meal plan from your catalog. This action cannot be undone.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 min-h-0 overflow-y-auto px-4 py-4 flex flex-col gap-4">
          <p className="text-sm text-base-content/70">
            Do you want to delete the meal plan <span className="font-semibold text-base-content">{mealPlan.name}</span>?
          </p>
